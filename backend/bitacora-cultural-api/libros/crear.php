...
// La identidad se obtiene exclusivamente de la sesión PHP.
$usuarioId = obtenerUsuarioAutenticado();

// Rechazar solicitudes sin un token válido.
verificarTokenCsrf();
...
foreach ($camposTexto as $campo => $longitudMaxima) {
    ...
    if (
        ...
        && (
            !is_string($valor)
            || mb_strlen(trim($valor)) > $longitudMaxima
        )
    ) {
        responder(422, [
            'estado' => 'error',
...
// Calificación opcional, entera entre 1 y 5.
...
try {
    ...
    // Comprobar que la cuenta de la sesión todavía existe.
    ...
    // Generar un UUID v4 para el nuevo libro.
    ...
    error_log('Error al crear el libro');

    responder(500, [
        'estado' => 'error',
        'mensaje' => 'No fue posible registrar el libro'
    ]);
}
