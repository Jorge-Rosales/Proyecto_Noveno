...
try {
    ...
    $consultaPelicula = $conexion->prepare(
        'SELECT id
         FROM peliculas
         WHERE id = :id
    ...
    $consultaPelicula->execute([
        'id' => $id,
        'usuario_id' => $usuarioId
    ]);

    if (!$consultaPelicula->fetch()) {
        responder(404, [
            'estado' => 'error',
    ...
    $pelicula = $consultaActualizada->fetch();
    ...
    error_log('Error de base de datos al actualizar la película');

    responder(500, [
        'estado' => 'error',
        'mensaje' => 'No fue posible actualizar la película'
    ]);

} catch (Throwable $error) {
    error_log('Error interno al actualizar la película');
